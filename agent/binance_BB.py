...
class TradingAgent:
    ...
    def fetch_market_data(self, limit: int = 5) -> pd.DataFrame:
        # Initialize Binance client
    ...
    def calculate_bb(self, data: pd.DataFrame) -> pd.DataFrame:
        bb = vbt.BBANDS.run(data['close'], window=2, alpha=2.9)
        ...
        # Check band crossovers
        buy_price = lower_band if prev_low < lower_band and current_high > lower_band else None
        sell_price = upper_band if prev_high > upper_band and current_low < upper_band else None
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self.precision = len(str(data['close'].iloc[0]))
            buy_price, sell_price = self.calculate_bb(data)
            ...
            if buy_price is None and sell_price is None and self.pending_orders:
                self.logger.debug(f"No valid trading opportunity {buy_price}, {sell_price}")
